...
func main() {
	...
	go func() {
		...
		nums := []int32{10, 20, 30}
		stream, err := client.Bi_Add(context.Background())
		if err != nil {
			l.Printf("--> Bi-Add, Error calling AddStream: %v", err)
		}
		...
		go func() {
			...
			for {
				res, err := stream.Recv()
				if err != nil {
					l.Printf("--> Bi-Add, Error receiving response: %v", err)
					break
				}
				l.Printf("--> Bi-Add, Received: %d\n", res.GetResult())
			}
		}()
		...
		for _, num := range nums {
			...
			err := stream.Send(req)
			l.Printf("--> Bi-Add, Send %d", num)
			if err != nil {
				l.Printf("--> Bi-Add, Failed to send request: %v", err)
			}
			time.Sleep(1 * time.Second)
		}
		err = stream.CloseSend()
		if err != nil {
			l.Printf("--> Bi-Add, Failed to receive response: %v", err)
		}
	}()
